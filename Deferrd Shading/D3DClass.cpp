						...
						float screenDepth, float screenNear)
{
	...
	// 렌더링을 위해 뷰포트를 설정합니다.
    m_viewport.Width = (float)screenWidth/2;
    m_viewport.Height = (float)screenHeight/2;
    m_viewport.MinDepth = 0.0f;
    m_viewport.MaxDepth = 1.0f;
    m_viewport.TopLeftX = 0.0f;
    m_viewport.TopLeftY = 0.0f;

	mBufferViewport[0].Width = (float)screenWidth / 2;
	mBufferViewport[0].Height = (float)screenHeight / 2;
	mBufferViewport[0].MinDepth = 0.0f;
	mBufferViewport[0].MaxDepth = 1.0f;
	mBufferViewport[0].TopLeftX = (float)screenWidth / 2;
	mBufferViewport[0].TopLeftY = 0.0f;

	mBufferViewport[1].Width = (float)screenWidth / 2;
	mBufferViewport[1].Height = (float)screenHeight / 2;
	mBufferViewport[1].MinDepth = 0.0f;
	mBufferViewport[1].MaxDepth = 1.0f;
	mBufferViewport[1].TopLeftX = 0.0f;
	mBufferViewport[1].TopLeftY = (float)screenHeight / 2;

	mBufferViewport[2].Width = (float)screenWidth / 2;
	mBufferViewport[2].Height = (float)screenHeight / 2;
	mBufferViewport[2].MinDepth = 0.0f;
	mBufferViewport[2].MaxDepth = 1.0f;
	mBufferViewport[2].TopLeftX = (float)screenWidth / 2;
	mBufferViewport[2].TopLeftY = (float)screenHeight / 2;
	...
	// 알파 비활성화 된 블렌드 상태 구조체를 만들려면 구조체를 수정하십시오.
	blendStateDescription.RenderTarget[0].BlendEnable = FALSE;

	// 구조체를 사용하여 두 번째 혼합 상태를 만듭니다.
...
void D3DClass::SetViewport(int num)
{
	if (num < 0 || num >2)
		return;

	m_deviceContext->RSSetViewports(1, &mBufferViewport[num]);
